...
inline void PutLengthPrefixedSlice(std::string* dst, const Slice& slice) {
    PutVarint32(dst, static_cast<uint32_t>(slice.size()));
    dst->append(slice.data(), slice.size());
}

inline const char* GetLengthPrefixedSlice(const char* p, const char* limit, Slice* result) {
    uint32_t len;
...
inline uint32_t Crc32cExtend(uint32_t crc, const char* data, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        crc = detail::kCrc32cTable[(crc ^ static_cast<uint8_t>(data[i])) & 0xFF] ^ (crc >> 8);
    }
