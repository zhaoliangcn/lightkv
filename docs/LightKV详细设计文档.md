...
slice.h 定义轻量级字节切片，不持有内存，仅包含指针和长度：
...
memtable.h 封装 SkipList 为 KV 接口：
...
block.h、block_builder.h 定义 Data Block 的编码格式。
...
bloom_filter.h 实现双重哈希布隆过滤器，默认 10 bits/key。
...
cache.h 实现基于 `(file_id, block_offset)` 的 LRU 缓存。
